import  ksk  from "../img/1ksk.mp4"
import  tep  from "../img/2tep.mp4"
import  nonstop  from "../img/3nonstop.mp4"
import  farmaGram  from "../img/4farmaGram.mp4"
import  tinte  from "../img/5tinte.mp4"


export const images = [
    {
        url: 'https://minilook.com.ar/wp-content/uploads/2015/04/logo-kosiuko.png',
        title: 'E-commerce',
        film: ksk,
        desc: 'E-commerce de la marca Kosiuko para proveedores mayoristas, realizado con react/node , con backend en asp.net core y SQL server',
        width: '33%',
    },
    {
        url: 'https://is1-ssl.mzstatic.com/image/thumb/Purple126/v4/80/f4/13/80f41357-cb12-5131-1985-19d63f40ab4d/AppIcon-0-0-1x_U007emarketing-0-0-0-10-0-0-sRGB-0-0-0-GLES2_U002c0-512MB-85-220-0-0.png/230x0w.webp',
        title: 'Production Corte',
        film: tep,
        desc: 'Desarrollo de Produccion de corte de la marca Kosiuko para integrar en Microsoft GP 2018, realizado con react/node , con backend en asp.net core y SQL server',
        width: '33%',
    },
    {
        url: 'https://www.nonstoptv.tv/media/__sized__/upload/pages/globalsettings/logons-1000px-1_k7gmnxk-thumbnail-300x300-2-thumbnail-300x300.png',
        title: 'T.E.P',
        film: nonstop,
        desc: 'Desarrollo de carga de clientes para la productora Nonstop, realizado con react/node , con backend en asp.net core y SQL server',
        width: '33%',
    },
    {
        url: 'https://pbs.twimg.com/profile_images/1348456981145776132/vVGo9SYF_400x400.jpg',
        title: 'Homepage',
        desc: 'Pagina principal de las consultoras Altiora y Implementaciones Industriales, realizado en HTML-CSS-Javascript',
        width: '33%',
    },
    {
        url: 'https://media.licdn.com/dms/image/C4E0BAQHxI_q20yKDsg/company-logo_200_200/0/1584535344147?e=1702512000&v=beta&t=Bpf3dIhSksfpUf8QqjdvegwROjy1dpcRvi4GA4MmzRw',
        title: 'Production Falla',
        film: tinte,
        desc: 'Desarrollo de Produccion de fallas de la empresa FarmaGram para integrar en Microsoft GP 2016, realizado con react/node , con backend en asp.net core y SQL server',
        width: '33%',
    },
    {
        url: 'https://media.licdn.com/dms/image/C4E0BAQHxI_q20yKDsg/company-logo_200_200/0/1584535344147?e=1702512000&v=beta&t=Bpf3dIhSksfpUf8QqjdvegwROjy1dpcRvi4GA4MmzRw',
        title: 'Commercial invoice',
        film: farmaGram,
        desc: 'Desarrollo de produccion para la facturacion de la empresa FarmaGram para integrar en Microsoft GP 2016, realizado con react/node , con backend en asp.net core y SQL server',
        width: '33%',
    },
];
